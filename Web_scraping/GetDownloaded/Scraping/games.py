...
class Game(scrapy.Spider):
    ...
    def temp(self, response):
        ...
        while k != 'stop':
            b_tag = response.xpath(
                '/html/body/div[1]/div/div[2]/div/div/div/div/div[1]/div/div[3]/div[' + str(n) + ']/div/h2/a/text()').extract_first()
            a_tag = response.xpath(
                '/html/body/div[1]/div/div[2]/div/div/div/div/div[1]/div/div[3]/div[' + str(n) + ']/div/h2/a/@href').extract_first()
    ...
    def temper(self, response):
        imgs = response.xpath(
            '//*[(@id = "primary-content")]//*[contains(concat( " ", @class, " " ), concat( " ", "author-admin", " " ))]').extract()
        download_link = response.xpath(
            '//*[(@id = "primary-content")]//form/@action').extract()
        download_data = response.xpath(
            '//*[(@id = "primary-content")]//form/input/@value').extract()
